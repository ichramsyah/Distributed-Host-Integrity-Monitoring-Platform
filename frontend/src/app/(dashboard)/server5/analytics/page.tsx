...
export default function Server5Analytics() {
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      ...
      setError(null);
      try {
        const [todayStats, historicalResponse, todayLogs] = await Promise.all([api('logs/analytics/', {}, 'server5'), api(`logs/analytics/historical/?days=${days}`, {}, 'server5'), api('logs/today/', {}, 'server5')]);
  ...
  const handleDateSelect = async (date: string) => {
    ...
    try {
      const [statsResponse, logsResponse] = await Promise.all([api(`logs/stats-by-date/?date=${date}`, {}, 'server5'), api(`logs/by-date/?date=${date}`, {}, 'server5')]);
