...
export default function Server2Analytics() {
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      ...
      setError(null);
      try {
        const [todayStats, historicalResponse, todayLogs] = await Promise.all([api('logs/analytics/', {}, 'server2'), api(`logs/analytics/historical/?days=${days}`, {}, 'server2'), api('logs/today/', {}, 'server2')]);
  ...
  const handleDateSelect = async (date: string) => {
    ...
    try {
      const [statsResponse, logsResponse] = await Promise.all([api(`logs/stats-by-date/?date=${date}`, {}, 'server2'), api(`logs/by-date/?date=${date}`, {}, 'server2')]);
