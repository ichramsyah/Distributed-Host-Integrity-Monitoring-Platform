...
export default function Server3Analytics() {
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      ...
      setError(null);
      try {
        const [todayStats, historicalResponse, todayLogs] = await Promise.all([api('logs/analytics/', {}, 'server3'), api(`logs/analytics/historical/?days=${days}`, {}, 'server3'), api('logs/today/', {}, 'server3')]);
  ...
  const handleDateSelect = async (date: string) => {
    ...
    try {
      const [statsResponse, logsResponse] = await Promise.all([api(`logs/stats-by-date/?date=${date}`, {}, 'server3'), api(`logs/by-date/?date=${date}`, {}, 'server3')]);
